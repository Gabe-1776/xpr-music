...
// One pull is one 2s batch (or the leftover second on pause/skip). Never a
// lumpy 8–16s catch-up: a late keeper bills at most `windowSec` this call.
// Pause at 1s into the batch bills 1s (`billable >= 1`), not "below window".
...
export class OndaStream extends Contract {
   ...
   */
  /**
   * Contract-owned 2s clock. Keeper pokes (no duration field).
   *   lastPull == 0 / no row → stamp now, charge 0 (open the batch)
   *   else billable = min(now - lastPull, windowSec); 1s pause remainder is
   *   just elapsed=1. Same-second poke is `fuse`. Late poke still max 2s.
   */
  @action("pullbal")
  pullbal(listener: Name, songId: string, token: ExtendedSymbol): void {
    ...
    const last: u32 = p == null ? 0 : p.lastPull;
    if (last == 0) {
    ...
    check(now > last, "fuse");
    let billable: u32 = now - last;
    if (billable > cfg.windowSec) billable = cfg.windowSec;
    check(billable >= 1, "below second");
  ...
  /**
   * Same 2s clock as pullbal, money from the listener's wallet.
   * lastPull==0 (fresh grant) opens the clock with no transfer.
   */
  @action("pullpay")
  pullpay(listener: Name, songId: string): void {
    ...
    check(now > g!.lastPull, "fuse");
    let billable: u32 = now - g!.lastPull;
    if (billable > cfg.windowSec) billable = cfg.windowSec;
    check(billable >= 1, "below second");

    const rate = this.requireRate(g!.tokenContract, g!.symRaw);
    let due: u64 = rate * <u64>billable;
  ...
  /**
   * Partial withdrawal — withdraw a SPECIFIC amount instead of the whole
   * piggy. `withdraw` (full drain) stays for the UI's withdraw-all.
   * Amount is raw integer units of the token (e.g. 5000 = 0.5000 XPR).
   * Keeps the row open with the remainder so the deposit cap (maxPerTick)
   * and the streaming clock (lastPull via pullbal) survive a partial
   * withdrawal — draining to zero removes the row and the next deposit
   * stamps a fresh cap at the then-current rate.
   */
  @action("withdrawamt")
  withdrawamt(listener: Name, token: ExtendedSymbol, amount: u64): void {
    requireAuth(listener);
    const symRaw = token.sym.raw();
    const id = tokenKey(listener, token.contract, symRaw);
    const row = this.balances.get(id);
    check(row != null, "no balance");
    check(amount > 0, "zero amount");
    check(row!.amount >= amount, "insufficient onda balance");
    if (row!.amount == amount) {
      // Exact drain: same as withdraw() — remove the row entirely.
      this.balances.remove(row!);
    } else {
      row!.amount -= amount;
      this.balances.update(row!, this.receiver);
