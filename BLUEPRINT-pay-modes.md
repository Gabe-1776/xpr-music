...
**Stream.** Keeper calls `pullbal(listener, songId, token)` every `windowSec`.
The contract debits `balances` and transfers that slice to the song's payout.
No lock, no buffer, no cap, **no re-sign** — the deposit is the only signature
until it runs out.

**Pause / skip.** Nothing to refund and nothing to reopen, so both are free by
construction.
...
Top-up is the default everywhere. The grant is offered only behind an advanced
toggle, with the exposure stated plainly and the cap visible.
