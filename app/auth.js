#!/usr/bin/env node
/**
 * Onda — wallet login (XPR Network identity proof) + session tokens.
 *
...
// Single-use proof-hash store, next to the canonical auth secret in
// catalog/. Was referenced but never defined — loadUsedProofs() swallowed
// the ReferenceError at boot, but claimProofUse() threw on every
// /api/auth/verify-proof call, breaking all wallet-extension logins
// (found via the verify-auth harness 401 "USED_FILE is not defined",
// 2026-09-02).
const USED_FILE = path.join(ROOT, "catalog", "used-proofs.json");
...
function claimProofUse(signatureHash) {
  ...
  // Proof hashes are retained permanently to prevent replay of non-expiring
  // identity proofs across cache eviction boundaries (closes ONDA-AUTH-002).
  // Memory footprint for 100k hashes is ~6MB.
