#!/usr/bin/env node
/**
 * Onda backend — plain `node:http` router (no web framework) with the
...
const PAY_SOURCES_FILE = path.join(ROOT, "catalog/pay-sources.json");
const RECENT_MAX = 40;
...
function checkAdminPin(req) {
  const pin = req && req.headers ? req.headers["x-admin-pin"] : null;
  if (typeof pin !== "string" || !pin || !ADMIN_PIN) return false;
  const pinBuf = Buffer.from(pin);
  const targetBuf = Buffer.from(ADMIN_PIN);
  if (pinBuf.length !== targetBuf.length) return false;
  return crypto.timingSafeEqual(pinBuf, targetBuf);
}
...
//
// Mirror of onda-pulse.js refreshModes(): one full-table read for ALL
// listeners, TTL-cached, never a per-listener query. Stores BOTH piggy
// banks (grant + balance); ondaModeFor() then applies the Settings
// pay_source (default top-up) so a leftover grant cannot hijack parked
// funds. Reads share the ONE process-wide RPC budget via rpc-budget.js.
const { rpcSlot: ondaRpcSlot } = require("./rpc-budget");
const ONDA_CONTRACT = "ondastream";
const ONDA_MODE_TTL_MS = Number(process.env.ONDA_MODE_TTL_MS || 60000);
let ondaModeMap = new Map();
let piggyMap = new Map(); // onda top-up piggy per account (playback balance)
let ondaModeAt = 0;
...
async function ondaRows(table) {
  await ondaRpcSlot();
  // THROWS on failure — an empty rows[] is indistinguishable from "this
  // listener has no balance row", which cached an empty piggyMap for the
  // full TTL and 409ed funded listeners on one transient RPC error. Callers
  // (refreshOndaModes via safe wrapper) decide how to handle the failure.
  // Paginated past 500 rows so listener #501 is not invisible.
  const out = [];
  let lower = "";
  for (let page = 0; page < MAX_TABLE_PAGES; page++) {
    const r = await fetch(`${NETWORK_RPC[appMode.network] || AGENTCORE_RPC}/v1/chain/get_table_rows`, {
      ...
      body: JSON.stringify({ json: true, code: ONDA_CONTRACT, scope: ONDA_CONTRACT, table, limit: 500, ...(lower ? { lower_bound: lower } : {}) }),
    });
    if (!r.ok) throw new Error(`ondastream ${table} rows: HTTP ${r.status}`);
...
async function refreshOndaModesSafe() {
  try {
    return await refreshOndaModes();
  } catch (e) {
    console.warn("onda mode read failed:", String((e && e.message) || e).slice(0, 160));
    return null; // caller keeps last-known modes; never cache an empty map
  }
}
...
async function refreshOndaModes() {
  ...
  const next = new Map();
  piggyMap = new Map(); // onda top-up piggy = the REAL playback balance
  function entry(name) {
  ...
  for (const g of await ondaRows("grants")) {
    if (Number(g.expiresAt) > now && Number(g.spent) < Number(g.budget)) {
      ...
      entry(g.listener).grant = { kind: "grant", contract: g.tokenContract, symbol: code, decimals: precision };
    }
  }
  for (const b of await ondaRows("balances")) {
    if (Number(b.amount) > 0) {
      ...
      entry(b.account).balance = { kind: "balance", contract: b.tokenContract, symbol: code, decimals: precision };
    }
  }
  ondaModeMap = next;
  ondaModeAt = Date.now();
...
/** Cached piggy (top-up) balance for display. Same read as the mode map. */
function ondaPiggyFor(actor) {
  return piggyMap ? piggyMap.get(actor) || null : null;
}
function invalidateOndaModes() {
  ondaModeAt = 0;
  piggyMap = new Map();
  try { ondaPulse.invalidateMode(); } catch (_) {}
}

// catalog/pay-sources.json — actor → "topup" | "wallet". Missing = topup.
// Never rsync catalog/; scp this file alone if it must move.
let paySourceCache = null;
function loadPaySources() {
  if (paySourceCache) return paySourceCache;
  try {
    const o = JSON.parse(fs.readFileSync(PAY_SOURCES_FILE, "utf8"));
    paySourceCache = o && typeof o === "object" && !Array.isArray(o) ? o : {};
  } catch { paySourceCache = {}; }
  return paySourceCache;
}
function savePaySources(map) {
  paySourceCache = map;
  fs.writeFileSync(PAY_SOURCES_FILE, `${JSON.stringify(map, null, 2)}\n`);
}
function normalizePaySource(v) {
  return v === "wallet" ? "wallet" : "topup";
}
function paySourceFor(actor) {
  if (!actor) return "topup";
  return normalizePaySource(loadPaySources()[actor]);
}

/** "grant" | "balance" | null — after Settings pay_source, not grant-first. */
async function ondaModeFor(actor) {
  if (!actor) return null;
  // Read failure -> null keeps the LAST-KNOWN cached modes (never an empty
  // map stamped fresh). "Cannot classify" — the keeper's chain reverts stay
  // the stop authority, and the /play piggy gate treats null as read-failed,
  // not as "no balance".
  const map = await refreshOndaModesSafe();
  if (map == null) return undefined; // signal: unknown, not "no mode"
  const entry = map.get(actor);
  if (!entry) return null;
  if (paySourceFor(actor) === "wallet") return entry.grant || null;
...
function tick(sess, { renew = true, now = Date.now() } = {}) {
  ...
  if (wasPlaying && !sess.playing) {
    console.warn("onda stop:", sess.actor || sess.id || "guest", JSON.stringify({
      reason: sess.stopReason || null,
      position: sess.position,
      escrow_usd: sess.escrow_usd,
      escrow_cur: sess.escrow_cur,
...
async function sessionState(sess) {
  ...
  return {
    ...
    // Settings switch: "topup" (default) or "wallet". Independent of whether
    // a grant/balance row currently exists. The keeper honors this, not
    // grant-first chain order.
    pay_source: sess.actor ? paySourceFor(sess.actor) : "topup",
    song: song ? (() => {
    ...
    // ONDA TOP-UP PIGGY — the actual playback balance the contract pulls
    // from (ondastream::balances). Separate from the wallet (liquid tokens):
    // topping up moves money from wallet → piggy. Raw integer units of the
    // piggy token (divide by 10^decimals for display).
    onda_topup: (() => {
      const p = sess.actor ? ondaPiggyFor(sess.actor) : null;
      return p ? {
        amount: p.amount,
        symbol: p.symbol,
        contract: p.contract,
        decimals: p.decimals,
        display: +((p.amount / Math.pow(10, p.decimals))).toFixed(6),
      } : null;
    })(),
...
const guarddog = {
  ...
  // actor -> { expiresAt: number, reason: string }
  timeouts: new Map(),
  // actor -> { count: number, errors: number, windowStart: number }
  velocity: new Map(),
  metrics: {
    inboundTotal: 0,
    errorTotal: 0,
...
function recordGuarddogActor(actor, isError) {
  if (!actor || typeof actor !== "string") return;
  const now = Date.now();
  let v = guarddog.velocity.get(actor);
  if (!v || now - v.windowStart > 60000) {
    v = { count: 1, errors: isError ? 1 : 0, windowStart: now };
    guarddog.velocity.set(actor, v);
  } else {
    v.count++;
    if (isError) v.errors++;
  }

  // Auto-timeout trigger: if enabled, >=100 req/min with >=50% errors
  if (guarddog.state.autoTimeout && v.count >= 100 && (v.errors / v.count) >= 0.5) {
    if (!guarddog.timeouts.has(actor)) {
      guarddog.timeouts.set(actor, {
        expiresAt: now + 15 * 60 * 1000,
        reason: `Auto-quarantine: ${v.count} req/min with ${Math.round((v.errors / v.count) * 100)}% errors`,
      });
      console.warn(`[guarddog] Auto-quarantined actor ${actor} for 15m (velocity: ${v.count}/min)`);
    }
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  guarddog.metrics.inboundTotal++;

  // Resolve actor for guarddog tracking if bearer or custom header present
  let reqActor = "";
  const authH = req.headers["authorization"] || "";
  const bMatch = authH.match(/^Bearer\s+(\S+)$/i);
  if (bMatch) {
    reqActor = auth.verifyToken(bMatch[1]) || "";
  }
  if (!reqActor && req.headers["x-account-actor"]) {
    reqActor = String(req.headers["x-account-actor"]);
  }
  if (reqActor) {
    recordGuarddogActor(reqActor, false);
  }

  // Guarddog Timeout check
  if (reqActor && guarddog.timeouts.has(reqActor)) {
    const t = guarddog.timeouts.get(reqActor);
    const now = Date.now();
    if (now >= t.expiresAt) {
      guarddog.timeouts.delete(reqActor);
    } else {
      recordGuarddogActor(reqActor, true);
      guarddog.metrics.errorTotal++;
      if (guarddog.state.tarpit) {
        guarddog.metrics.tarpitCount++;
        await new Promise((resolve) => setTimeout(resolve, 5000));
      }
      const retrySec = Math.ceil((t.expiresAt - now) / 1000);
      return send(res, 429, {
        error: "actor_timeout",
        retry_after_seconds: retrySec,
        reason: t.reason || "Actor in penalty box cooldown",
      });
    }
  }

  // Guarddog Master Circuit Breaker check on play/pulse/topup payment endpoints
  if (guarddog.state.circuitBreaker && (p.startsWith("/api/session/") || p === "/api/account/topup")) {
    return send(res, 503, {
      error: "maintenance_mode",
      message: "Guarddog Master Circuit Breaker engaged. Payments and playback temporarily frozen.",
    });
  }
  ...
  if (p === "/api/session/currency" && req.method === "POST") {
    ...
    if (sess.playing && isPaymentEligible(song)) {
      // Same piggy gate as /play: the top-up pool is what pays, not the
      // liquid wallet copy. Prime-first + unknown-passthrough, same as /play.
      const modes = await refreshOndaModesSafe();
      const readFailed = modes == null;
      const piggy = readFailed ? undefined : ondaPiggyFor(sess.actor);
      if (piggy !== undefined && (!piggy || !(piggy.amount > 0))) {
        sess.playing = false;
        sess.stopReason = { reason: "insufficient_balance", currency: cur, topup_required: true };
        invalidateOndaModes();
        flushSessionMetrics(sess);
      }
    }
  ...
  if (p === "/api/session/play" && req.method === "POST") {
    ...
    if (body.song_id && body.song_id !== sess.songId) {
      // Song switch: pull leftover seconds of the OLD track on-chain, then
      // start a new 2s batch against the new payout.
      if (sess.playing && sess.actor && sess.songId) {
        try { await ondaPulse.settleRemainder(sess.actor, sess.songId, paySourceFor(sess.actor)); } catch (e) {
          console.warn("onda remainder on skip", String((e && e.message) || e).slice(0, 160));
        }
      }
      rebateHold(sess);
      flushSessionMetrics(sess);
    ...
    // No money, no billable music. The gate is the ONDA TOP-UP PIGGY (the
    // balance the contract actually pulls) — NOT the liquid wallet copy,
    // which is a display number. piggy amount comes from the cached mode
    // read; the keeper's chain classification remains the real stop.
    if (isPaymentEligible(wanted) && sess.actor) {
      // PRIME: the first play after restart has an empty piggyMap until the
      // first mode read lands. Priming here (safe, TTL-throttled) closes the
      // funded-listener-409 window. A null modes map = read failed = unknown;
      // do not block on unknown — the keeper's chain revert decides.
      const modes = await refreshOndaModesSafe();
      const readFailed = modes == null;
      const piggy = readFailed ? undefined : ondaPiggyFor(sess.actor);
      if (piggy !== undefined && (!piggy || !(piggy.amount > 0))) {
        sess.playing = false;
        sess.stopReason = { reason: "insufficient_balance", currency: sess.currency };
        invalidateOndaModes(); // a fresh deposit must be visible on retry
        return send(res, 409, { error: "insufficient balance", currency: sess.currency, balance: piggy ? piggy.amount : 0, topup_required: true });
      }
    } else if (!isPaymentEligible(wanted)) {
      rebateHold(sess);
    }
    sess.stopReason = null;
    // Billable Originals: poke FIRST. Never return 200 / start audio if the
    // selected piggy bank cannot actually pay (missing grant, missing
    // linkauth, empty top-up). settleRemainder returns a reason string on
    // a real pay failure and null on success / fuse.
    if (isPaymentEligible(wanted) && sess.actor && sess.songId) {
      const src = paySourceFor(sess.actor);
      const mode = await ondaModeFor(sess.actor);
      if (mode === undefined) {
        // Read failed — cannot classify. Do NOT 409 a funded listener on a
        // transient RPC error; let the keeper's chain revert decide.
        console.warn("onda mode unknown on play (read failed) — proceeding for", sess.actor);
      } else if (!mode) {
        sess.playing = false;
        sess.payment_error = src === "wallet" ? "unfunded" : "nofunds";
        invalidateOndaModes();
        return send(res, 409, { error: "unpaid", payment_error: sess.payment_error });
      }
      let reason = null;
      try {
        reason = await ondaPulse.settleRemainder(sess.actor, sess.songId, src);
      } catch (e) {
        console.warn("onda clock poke on play", String((e && e.message) || e).slice(0, 160));
      }
      if (reason) {
        sess.playing = false;
        sess.payment_error = reason;
        return send(res, 409, { error: "unpaid", payment_error: reason });
      }
    }
    sess.playing = true;
  ...
  if (p === "/api/onda/wallet" && req.method === "GET") {
    ...
    // Bearer fallback — a persisted session id dies on server restart
    // (in-memory session map) while the 7-day auth token stays valid.
    // Without this, every phone that survived a restart got
    // {reliable:false} and the wallet card rendered "—" everywhere
    // (Gabriel, 2026-09-03).
    const bearerActor = sess && sess.actor ? null : auth.verifyToken((req.headers["authorization"] || "").match(/^Bearer\s+(\S+)$/i)?.[1] || "");
    const actor = (sess && sess.actor) || bearerActor || (url.searchParams.get("actor") || "").trim();
    if (!actor) return send(res, 200, { actor: null, tokens: [], reliable: false });
    try {
      return send(res, 200, await ondaWallet.walletFor(actor, { fresh: url.searchParams.get("fresh") === "1" }));
    } catch (e) {
  ...
  if (p === "/api/session/pay-source" && req.method === "POST") {
    const sid = req.headers["x-session-id"];
    const sess = sid && sessions.get(sid);
    if (!sess) return send(res, 401, { error: "no session" });
    const actor = bindSessionActor(req, sess);
    if (!actor) return send(res, 401, { error: "wallet login required" });
    const body = await readBody(req);
    if (body && body.__too_large) return send(res, 413, { error: "too large" });
    const source = normalizePaySource(body && body.source);
    const map = loadPaySources();
    map[actor] = source;
    savePaySources(map);
    invalidateOndaModes();
    return send(res, 200, await sessionState(sess));
  ...
  if (p === "/api/session/pause" && req.method === "POST") {
    ...
    if (sess.playing && sess.actor && sess.songId) {
      try { await ondaPulse.settleRemainder(sess.actor, sess.songId, paySourceFor(sess.actor)); } catch (e) {
        console.warn("onda remainder on pause", String((e && e.message) || e).slice(0, 160));
      }
    }
    sess.playing = false;
  ...
  if (p === "/api/playlists" && req.method === "POST") {
    ...
    const body = await readBody(req);
    if (!scope) return send(res, 401, { error: "no session" });
    const gate = actorGate(scope) || writeGate(scope);
  ...
  // Albums: artists create albums and assign songs to them. Ownership is
  // derived from the verified Bearer token, matching PUT and DELETE below
  // (closes ONDA-API-001).
  if (p === "/api/albums" && req.method === "POST") {
    ...
    if (!name) return send(res, 400, { error: "album name is required" });
  ...
  if (p === "/api/submissions" && req.method === "POST") {
    ...
    const isAdminPin = checkAdminPin(req);
  ...
  if (p === "/api/metrics") {
    if (!checkAdminPin(req)) return send(res, 401, { error: "admin pin required" });
    const m = loadMetrics();
  ...
  // ── Guarddog Admin API ──────────────────────────────────────────────
  if (p === "/api/admin/guarddog" && req.method === "GET") {
    if (!checkAdminPin(req)) return send(res, 401, { error: "admin pin required" });
    const now = Date.now();
    const uptimeSec = Math.max(1, Math.round((now - guarddog.metrics.startedAt) / 1000));
    const reqRate = Number((guarddog.metrics.inboundTotal / uptimeSec).toFixed(2));

    const watchlist = [];
    for (const [actor, v] of guarddog.velocity.entries()) {
      if (now - v.windowStart <= 120000) {
        const isTimedOut = guarddog.timeouts.has(actor);
        const tInfo = isTimedOut ? guarddog.timeouts.get(actor) : null;
        watchlist.push({
          actor,
          rpm: v.count,
          errorRate: v.count > 0 ? Number((v.errors / v.count).toFixed(2)) : 0,
          status: isTimedOut ? "timed_out" : "healthy",
          expiresIn: tInfo ? Math.max(0, Math.ceil((tInfo.expiresAt - now) / 1000)) : 0,
          reason: tInfo ? tInfo.reason : "",
        });
      }
    }
    for (const [actor, tInfo] of guarddog.timeouts.entries()) {
      if (!watchlist.find((w) => w.actor === actor)) {
        watchlist.push({
          actor,
          rpm: 0,
          errorRate: 1.0,
          status: "timed_out",
          expiresIn: Math.max(0, Math.ceil((tInfo.expiresAt - now) / 1000)),
          reason: tInfo.reason,
        });
      }
    }
    ...
    return send(res, 200, {
      state: guarddog.state,
      metrics: {
        inboundReqRate: reqRate,
        tarpitCount: guarddog.metrics.tarpitCount,
        timedOutActors: guarddog.timeouts.size,
      },
      watchlist,
    });
  }

  if (p === "/api/admin/guarddog/toggle" && req.method === "POST") {
    if (!checkAdminPin(req)) return send(res, 401, { error: "admin pin required" });
    const body = await readBody(req);
    const key = body.key;
    if (typeof guarddog.state[key] === "undefined") {
      return send(res, 400, { error: "invalid toggle key" });
    }
    guarddog.state[key] = Boolean(body.value);
    console.log(`[guarddog] Toggle '${key}' updated to: ${guarddog.state[key]}`);
    return send(res, 200, { ok: true, state: guarddog.state });
  }

  if (p === "/api/admin/guarddog/timeout" && req.method === "POST") {
    if (!checkAdminPin(req)) return send(res, 401, { error: "admin pin required" });
    const body = await readBody(req);
    const actor = typeof body.actor === "string" ? body.actor.trim() : "";
    if (!actor) return send(res, 400, { error: "actor required" });
    const minutes = Number(body.minutes) || 15;
    const reason = typeof body.reason === "string" && body.reason ? body.reason : "Admin manual quarantine";
    guarddog.timeouts.set(actor, {
      expiresAt: Date.now() + minutes * 60 * 1000,
      reason,
    });
    return send(res, 200, { ok: true, actor, minutes });
  }

  if (p === "/api/admin/guarddog/lift-timeout" && req.method === "POST") {
    if (!checkAdminPin(req)) return send(res, 401, { error: "admin pin required" });
    const body = await readBody(req);
    const actor = typeof body.actor === "string" ? body.actor.trim() : "";
    if (!actor) return send(res, 400, { error: "actor required" });
    guarddog.timeouts.delete(actor);
  ...
  if (p === "/llms.txt") return serveStatic(res, req, "web/llms.txt");
  if (p === "/admin/llms.txt") return serveStatic(res, req, "web/admin-llms.txt");
...
server.listen(PORT, () => {
  console.log(`🎵 Onda backend on http://127.0.0.1:${PORT}  (desktop: / · mobile: /mobile)`);
  ondaPulse.start(() => {
    const live = [];
    for (const sess of sessions.values()) {
      if (sess.playing && sess.actor && sess.songId) {
        live.push({
          actor: sess.actor,
          songId: sess.songId,
          paySource: paySourceFor(sess.actor),
          billable: isPaymentEligible(songFor(sess)),
        });
      }
    }
    return live;
