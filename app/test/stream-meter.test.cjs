...
test("stale 10s heartbeat vests at most the open 2s hold, does not bill 10s, no rebate", () => {
  const { sess, wallet } = fresh({ lastTick: 0 });
  sess.playing = true;
  m.openPlay(sess, wallet);
  const held = sess.escrow_usd;
  m.tick(sess, { now: 10_000, eligible: true, duration: 105, wallet, renew: true });
...
test("explicit pause after a 10s gap rebates the hold (stop is not a crash)", () => {
  const { sess, wallet } = fresh({ lastTick: 0 });
  sess.playing = true;
  m.openPlay(sess, wallet);
  m.tick(sess, { now: 10_000, eligible: true, duration: 105, wallet, renew: false });
  const rebated = m.closePlay(sess, wallet);
  sess.playing = false;
  assert.equal(sess.spend_usd, 0);
  assert.equal(sess.escrow_usd, 0);
...
test("window is 2 seconds", () => {
  assert.equal(m.WINDOW_S, 2);
  assert.ok(Math.abs(m.windowUsd() - 0.0001) < 1e-15);
});
