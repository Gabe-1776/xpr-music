...
try {
  await api("/api/session/play", { method: "POST", body: { song_id: "signal-bloom", position: 0 }, headers: guestHeaders });
} catch (e) {
...
await api("/api/library/save", { method: "POST", body: { song_id: "signal-bloom" }, headers: authHeaders });
const accountLib = await api("/api/library", { headers: authHeaders });
const accountSaved = (accountLib.saved || []).map((s) => s.id);
check("account-scoped library persists", accountSaved.includes("signal-bloom"), `saved=${accountSaved.join(",") || "(empty)"}`);

const guestLib = await api("/api/library", { headers: guestHeaders });
const guestSaved = (guestLib.saved || []).map((s) => s.id);
check("guest library is isolated from account favorites", !guestSaved.includes("signal-bloom"), `guest=${guestSaved.join(",") || "(empty)"}`);

// 4b. Wallet session CAN start playback.
const walletPlay = await api("/api/session/play", {
  method: "POST",
  body: { song_id: "signal-bloom", position: 0 },
  headers: { ...guestHeaders, ...authHeaders },
});
check("wallet session can play", walletPlay.playing === true && walletPlay.song?.id === "signal-bloom", `song=${walletPlay.song?.id}`);
...
await api(`/api/playlists/${playlistId}/add`, { method: "POST", body: { song_id: "night-ledger" }, headers: authHeaders });
const playlistDetail = await api(`/api/playlists/${playlistId}`, { headers: authHeaders });
check(
  "account-scoped playlist persists with song",
  playlistDetail.playlist.song_ids.includes("night-ledger") && playlistDetail.playlist.songs.length === 1,
  `song_ids=${playlistDetail.playlist.song_ids.join(",")}`,
);

// 6. Cleanup: remove favorite + delete playlist so the account store is tidy.
await api("/api/library/remove", { method: "POST", body: { song_id: "signal-bloom" }, headers: authHeaders });
