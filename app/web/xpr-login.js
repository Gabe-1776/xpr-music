...
(function (global) {
  ...
  /** Partial withdrawal — withdraw a specific raw amount from the piggy.
   * amountRaw is raw integer units (XPR: 4 decimals, XUSDC: 6). The contract
   * action is `withdrawamt`; full drain stays on `withdraw`. */
  async function ondaWithdrawAmount(o) {
    const token = requireOndaToken((o || {}).tokenId);
    const amountRaw = Math.floor(Number((o || {}).amountRaw));
    if (!Number.isFinite(amountRaw) || amountRaw <= 0) throw new Error("amount required");
    return signAction(function (session) {
      return {
        account: chainConfig.contract,
        name: "withdrawamt",
  ...
  async function ondaGrant(o) {
    ...
    // Read the account's CURRENT permissions/links before opening a wallet
    // prompt. WebAuth's phone app silently refuses to DISPLAY updateauth/
    // linkauth (proven 2026-07-11) — so if `ondapull` already exists with
    // ondastream@eosio.code, send ONLY `ondastream::grant`. That is a normal
    // contract action and the wallet will actually open.
    const info = await getAccountInfo(actor);
    const perms = info.permissions || [];
    let ondaReady = false;
    const linked = [];
    perms.forEach(function (p) {
      if (p && p.perm_name === PERM) {
        const accts = (p.required_auth && p.required_auth.accounts) || [];
        ondaReady = accts.some(function (a) {
          const pl = a && a.permission;
          return pl && String(pl.actor) === CONTRACT && String(pl.permission) === "eosio.code";
        });
      }
      (p.linked_actions || []).forEach(function (l) {
        if (l && l.account === token.contract && l.action === "transfer") linked.push(p.perm_name);
      });
    });
    ...
    const build = function (session) {
      ...
      } else if (linked.length && linked.indexOf(PERM) < 0) {
        // A DIFFERENT perm is visibly linked to this token's transfer — swap it.
        // Empty `linked_actions` on testnet nodes is common and is NOT proof
        // the link is missing; stuffing linkauth into that case is what made
        // WebAuth swallow the request (felixpaw already has ondapull).
        actions.push({
          account: "eosio",
          name: "unlinkauth",
          authorization: [session.auth],
      ...
      await Promise.race([
        signAction(function (session) {
          return {
            ...
            name: "linkauth",
            authorization: [session.auth],
            data: { account: actorOf(session), code: token.contract, type: "transfer", requirement: PERM },
          };
        }),
        new Promise(function (_, reject) {
          setTimeout(function () {
            reject(Object.assign(new Error("link timeout"), { code: "E_GRANT_TIMEOUT" }));
          }, 25000);
        }),
      ]);
    } catch (err) {
  ...
  /** The INVERSE of ondaUnlinkauth: re-link eosio.token::transfer to ondapull.
   * Standalone so a listener can re-establish the link without re-running the
   * whole grant form (which demands a budget even when a grant is already
   * active). One linkauth action, no updateauth — bundling made WebAuth hide
   * the prompt (proven 2026-07-11). */
  async function ondaLinkauth(tokenId) {
    const token = requireOndaToken(tokenId);
    return signAction(function (session) {
      return {
        account: "eosio",
        name: "linkauth",
        authorization: [session.auth],
        data: { account: actorOf(session), code: token.contract, type: "transfer", requirement: "ondapull" },
      };
    });
  }
  ...
  global.ondaChain = {
    ...
    ondaWithdrawAmount: ondaWithdrawAmount,
    ondaGrant: ondaGrant,
    ondaRevoke: ondaRevoke,
    ondaUnlinkauth: ondaUnlinkauth,
    ondaLinkauth: ondaLinkauth,
  };
})(window);
