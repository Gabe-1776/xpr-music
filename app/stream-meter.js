...
const WINDOW_S = 2;
const STALE_GRACE_S = Number(process.env.ONDA_STALE_GRACE_S || 10); // client polls every 2s; grace must absorb browser tab-throttling and server poll latency spikes. Was 1 (killed streams at ~15s), then 4 (still stale_hold at ~69s — server log 2026-08-30T18:14:05Z). 10s of no polls while audio keeps advancing = genuinely dead.
...
function tick(sess, opts) {
  ...
  if (played + 1e-12 < wantPlay) {
    // Wallet-copy shortfall is NOT a stop for wallet-linked sessions: the
    // chain piggy is what pays, and the keeper's classification (nofunds /
    // budget / nolink from pull reverts) is the only stop authority. The
    // meter's copy is display-only. (Ghost stops from this branch killed
    // streams at 7s and 15s on 2026-08-30.)
    if (!sess.actor) {
      sess.playing = false;
      sess.stopReason = { reason: "insufficient_balance", currency: curOf(sess) };
      rebate(sess, wallet, persist);
    } else {
      // Actor session: keep playing AND advance the playhead by the wall
      // time. The chain piggy pays per its own 2s pulls — if we froze
      // position here, the client's 2s poll would rewind audio.currentTime
      // into a stutter/rewind loop while the piggy still pays on-chain.
      // Display accrual only: no escrow/spend/wallet mutation.
      sess.position = prev + wantPlay;
    }
    ...
    fillHold(sess, wallet, persist);
    // Same delegation: an empty display copy must not stop a wallet session.
    if (sess.escrow_usd <= 0 && !sess.actor) {
      sess.playing = false;
      sess.stopReason = { reason: "insufficient_balance", currency: curOf(sess) };
    }
  }
